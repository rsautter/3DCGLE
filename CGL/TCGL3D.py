...
class TCGL():
	...
	Wrote by: Rubens Andreas Sautter (2025)
	...
	def __init__(self, c1=1.5, c2=0.0,h=1.0, msize = 128, ic='r',a0=1.0,b0=0.0,dim=2,device='cpu'):
		'''
		Spatial parameters:
	...
	def __getRandom(self,n,dim):
		...
		return torch.rand(n**dim).reshape(newShape).to(self.device)

	def __getGaussian(self,n,dim):
		out = torch.zeros(torch.repeat(n,dim))
		c = n/2
		squareDists = torch.sum((torch.indices(out.shape)-c)**2,axis=0)
		return (2*(torch.exp(-squareDists/(20*n))-0.5)).to(self.device)
	...
	def getInitialCondition(self):
		...
		return torch.Tensor(self.a).to(self.device)
	...
	def reaction(self, a, t):
		return a - (1+1j*self.c2)*(torch.abs(a)**2)*a
	...
	def solveRKF45(self,dt,ntimes,stepsave,dtTolerace=1e-5):
		...
		for time in tqdm.tqdm(range(ntimes)):
			while t < time*dt:
				...
				error = torch.max(torch.abs(approach4-approach5))
				if error> dtTolerace:
	...
	def timeDerivatives(self,state,time):
		rFtState = fftn(torch.real(state)).to(self.device)
		iFtState = fftn(torch.imag(state)).to(self.device)

		# spectral shift variables
		specR = torch.zeros(state.shape).to(self.device)
		specI = torch.zeros(state.shape).to(self.device)
		...
		for i in range(len(state.shape)):
			...
			seq = torch.ones(len(state.shape)).int().to(self.device)
			fx  = 2*np.pi*fftfreq(state.shape[i]).to(self.device)
			seq[i] = len(fx)
			fx = fx.reshape(*tuple(seq.tolist()))
		...
		lap  =    torch.real(ifftn(specR)) + 1j*torch.real(ifftn(specI))/(self.h**2)
		return (1+self.c1*1j)*lap + self.reaction(state,time)
